...
export default function DocumentDetailPage() {
    ...
    async function performAction() {
        ...
        if (
            (actionModal === "reject" || actionModal === "revision") &&
            !actionComment.trim()
        ) {
    ...
    const canAct =
        isCurrentApprover &&
        ["PENDING_APPROVAL", "IN_REVIEW"].includes(document.status);
    const isSubmitter = document.submittedBy?.id === userId;
    const canEdit =
        isSubmitter &&
        ["DRAFT", "REVISION_REQUESTED"].includes(document.status);
    const canSubmit =
        isSubmitter &&
        ["DRAFT", "REVISION_REQUESTED"].includes(document.status) &&
        document.templateId;
    ...
    return (
        <div className="max-w-6xl mx-auto animate-fade-in">
            ...
            <div className="flex items-start gap-4 mb-6">
                <Link
                    href="/documents"
                    className="p-2 rounded-xl hover:bg-muted transition-colors text-muted-foreground shrink-0 mt-0.5"
                >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 space-y-5">
                    ...
                    {canApprove &&
                        ["PENDING_APPROVAL", "IN_REVIEW"].includes(
                            document.status,
                        ) && (
                                ...
                                <div className="flex items-start gap-3 mb-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm text-amber-700 dark:text-amber-400">
                                            Step {document.currentStepIndex + 1}
                                            :{" "}
                    ...
                    <div className="flex items-center gap-1 bg-muted/50 rounded-xl p-1 border border-border w-fit">
                        {(["content", "comments", "history"] as Tab[]).map(
                            (tab) => (
                                <button
                                    key={tab}
                        ...
                        <div className="bg-card border border-border rounded-2xl overflow-hidden">
                            {document.content ? (
                                ...
                                <div className="p-12 text-center text-muted-foreground">
                                    ...
                                    <p className="text-sm">
                                        No written content — document may have
                                        attached files.
                                    </p>
                                </div>
                            )}
                                                ...
                                                className={cn(
                                                    ...
                                                    isComplete
                                                        ...
                                                        : isRejected
                                                          ...
                                                          : isCurrent
                                                            ? "bg-amber-100 border-amber-400 text-amber-700 dark:bg-amber-900/30"
                        ...
                        <dl className="space-y-3">
                            {[
                                ...
                                },
                                {
                                    label: "Department",
                                    value:
                                        document.submittedBy?.department
                                            ?.name ?? "—",
                                },
                                {
                                    label: "Created",
            ...
            <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {comment.isInternal && (
                            <span className="text-[10px] font-medium bg-violet-100 text-violet-700 dark:bg-violet-900/30 dark:text-violet-400 px-2 py-0.5 rounded-full">
                                Internal
                            </span>
                        )}
            ...
            {logs.map((log, i) => {
                ...
                return (
                    ...
                    >
                        <div
                            className={cn(
                                "w-8 h-8 rounded-full flex items-center justify-center text-sm shrink-0 z-10 border-2 border-background",
                                meta.color,
                            )}
                        >
                            {meta.emoji}
